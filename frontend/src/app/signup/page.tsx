...
type PhoneStatus = 'idle' | 'checking' | 'taken' | 'available';
...
const SignupPage = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setPhoneStatus(response.exists ? 'taken' : 'available');
      } catch {
        setPhoneStatus('idle');
      }
    }, 600);
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await connectToBackendServices.otpRequest({ flow: 'signup', phoneNumber });
      if (response?.success) {
        setStep(2);
      } else {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await connectToBackendServices.verifyOTPSignup({ name, phoneNumber, otp });
  ...
  const phoneBorderClass =
    phoneStatus === 'taken' ? 'border-red-400 focus:ring-red-400' :
    phoneStatus === 'available' ? 'border-green-400 focus:ring-green-400' :
    'border-slate-300 focus:ring-blue-500';

  if (authContext?.isAuthenticated) return null;

  return (
    <div className="text-black font-semibold min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col justify-center px-6 py-12">
          ...
          {step === 1 ? (
            <form className="space-y-8" onSubmit={handleRequestOTP}>
              <div>
                ...
                <input
                  ...
                  className="block w-full px-4 py-4 border border-slate-300 rounded-xl shadow-sm text-base placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-medium">
                    ...
                    {phoneStatus === 'taken'    && <span className="text-red-500">✗ Already registered</span>}
                    {phoneStatus === 'available'&& <span className="text-green-500">✓ Available</span>}
                  </span>
                </div>
                {/* Hint below input */}
                {phoneStatus === 'taken' && (
              ...
              <button
                type="submit"
                disabled={loading || phoneNumber.length !== 10 || name.trim().length < 2 || phoneStatus === 'taken' || phoneStatus === 'checking'}
            ...
            <form className="space-y-8" onSubmit={handleVerifyOTP}>
              <div className="text-base text-slate-600 text-center">
                ...
                <button type="button" onClick={() => setStep(1)} className="ml-2 text-blue-600 hover:underline">
                  Edit
                </button>
              </div>
              <div>
